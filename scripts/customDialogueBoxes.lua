...
function onStartCountdown()
	...
	if not allowCountdown then
        if checkFileExists('data/'..string.lower(string.gsub(songName, "%s+", "-"))..'/dialogue.json') then
...
function onCustomSubstateCreate()
    ...
    dialogueJSON = callMethodFromClass('tjson.TJSON', 'parse', {getTextFromFile('data/'..string.lower(string.gsub(songName, "%s+", "-"))..'/dialogue.json')})

    if dialogueJSON.dialogue[page].use_box == nil then
        user32.MessageBoxA(nil, 'use_box returned nil in "data/'..string.lower(string.gsub(songName, "%s+", "-"))..'/dialogue.json".', 'Error: Dialogue JSON Error', ffi.C.MB_OK + ffi.C.MB_ICONINFORMATION)
        exitCustomDialogue()
        return
    end
